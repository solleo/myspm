...
%  .dur_sec    {1xR} cell array of durations over R runs
...
SPM = [];
SPM.xBF = spm_get_bf(...
  struct('T',job.fMRI_T ,'T0',job.fMRI_T0, ...
    'dt',job.tr_sec/job.fMRI_T0, 'name','hrf'));
SPM.Sess = [];
SPM.nscan = num_frames;
for irun = 1:num_runs
  ...
  xX{irun} = X{irun}(...
    (0:(num_frames(irun) - 1))*job.fMRI_T + job.fMRI_T0 + 32,:);
