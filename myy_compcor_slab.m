...
dir1=fullfile(EXP.dir_fs,subjid,meastype);
dir2=fullfile(EXP.dir_fs,subjid,meastype,'compcor');
[~,~]=mkdir(dir2);
cd(dir2);
...
for r=1:numruns
  ...
  %% 0. bring epi files
  fname_epi = fullfile(dir1,['au',meastype(end-2:end),ridx,'.nii']);
  fname_epi2 = fullfile(dir2,['au',meastype(end-2:end),ridx,'.nii']);
  unix(['ln -sf ',fname_epi,' ',fname_epi2]);
  fname_epi = fname_epi2;
  
  %% 0. bring rp files
  EXP.fname_rp=[dir2,'/rp_',meastype(end-2:end),ridx,'.txt'];
  dir3=['/scr/vatikan1/skim/Tonotopy/main/',subjid,'/7T/func/'];
  copyfile([dir3,'rp_',meastype(end-2:end),ridx,'.txt'], EXP.fname_rp);
  
  %% 1. define EPI brain stringently (thus not affected by dropped signals)
  fname_meanepi   = fullfile(dir1,['meanau',meastype(end-2:end),ridx,'.nii']);
  fname_mepibrain = fullfile(dir1,['meanau',meastype(end-2:end),ridx,'_brain.nii']);
  ...
  exp1.name_moving=['/scr/vatikan1/skim/Tonotopy/main/',subjid,'/7T/UNI.nii'];
  for c=1:3
    exp1.name_others{c}=['/scr/vatikan1/skim/Tonotopy/main/', ...
      subjid,'/7T/c',num2str(c),'UNI.nii'];
  end
  exp1.prefix='fs';
  if ~exist(['/scr/vatikan1/skim/Tonotopy/main/',subjid, ...
      '/7T/fsc',num2str(c),'UNI.nii'],'file')
    myspm_coreg(exp1);
  end
  ...
  if ~exist(fullfile(dir2,['c',num2str(c),'UNI_run',ridx,'.nii']), 'file')
    fname_dat = fullfile(dir1,'bbr',['boldloc_to_t1w_run',ridx,'.dat']);
    for c=1:3
      fname_tpm = ['/scr/vatikan1/skim/Tonotopy/main/',subjid, ...
        '/7T/fsc',num2str(c),'UNI.nii'];
      fname_tpm_epi= fullfile(dir2,['c',num2str(c),'UNI_run',ridx,'.nii']);
  ...
  EXP.fname_masks  = {fullfile(dir2,['c2UNI_run',ridx,'.nii']), ...
    fullfile(dir2,['c3UNI_run',ridx,'.nii'])};
  EXP.fname_gmmask = fullfile(dir2,['c1UNI_run',ridx,'.nii']);
  ...
  [path1,f1,e1]=fileparts(fname_epi);
  EXP.name_epi = [f1,e1];
...
if PCNum
  ...
  PCs = U(:,1:PCNum);
  PCs = double(PCs);
  save(fullfile(path1,['cc_',boldtxt,'_wmcsf',tprob{3},output_suffix,'_eigenval.txt']), 'eigval', '-ASCII', '-DOUBLE','-TABS')
else
  % mean
  PCs = mean(AllVolume,2);
  eigval = [];
end
save(fullfile(path1,['cc_',boldtxt,'_wmcsf',tprob{2},output_suffix,'_eigenvec.txt']), 'PCs', '-ASCII', '-DOUBLE','-TABS')
save(fullfile(path1,['cc_',boldtxt,'_gm',tprob{1},'.txt']), 'gm', '-ASCII', '-DOUBLE','-TABS')
...
if ~isfield(EXP,'nofigure')
  ...
  [~,res] = mydir(fullfile(path1,['art_out_mov_',name1,'*']));
  load (res{1},'R');
  load(fullfile(path1,['cc_',boldtxt,'_gm',tprob{1},'.txt']), 'gm');
  load(fullfile(path1,['cc_',boldtxt,'_wmcsf',tprob{2},output_suffix,'_eigenvec.txt']),'PCs');
