function job = myspm_import_ft(job)
...
% % Correct MEG channel units with the sensor units:
% [~,idx] = ismember({D.channels.label}, D.sensors.meg.label);
% for i = 1:numel(idx)
%   D.channels(i).units = D.sensors.meg.chanunit{idx(i)};
% end
