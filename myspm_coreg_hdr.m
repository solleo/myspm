function EXP = myspm_coreg_hdr(EXP)
% EXP = myspm_coreg_hdr(EXP)
...
EXP.fname_epi_unbiased = fname_epi_unbiased;
...
estimate1=[];
estimate1.ref{1}    = EXP.fname_t1w;
estimate1.source{1} = fname_epi_unbiased;
% ------------------here you specified OTHER imgaes-----------------
estimate1.other{1} = EXP.fname_epi; % NIFTI-format have only ONE xfm for all volumes (no need to put all frames here)
% ------------------------------------------------------------------
...
if ~exist(fname_out,'file')
  ...
  warning('This function CHANGES the header of %s',EXP.fname_epi);
...
if ~exist([p4,'/r',f4,e4],'file')
  matlabbatch={};
  matlabbatch{1}.spm.spatial.coreg.write.ref{1}    = EXP.fname_t1w;
