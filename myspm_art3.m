...
fname_art=[path1,'/art_out_mov_',name1,output_suffix,ridx,'.mat'];
...
if ~exist(fname_art,'file') || ~~overwrite
  ...
  movefile([path1,'/art_regression_outliers_',name1,'.mat'], ...
    [path1,'/art_out_',name1,output_suffix,ridx,'.mat']);
  movefile([path1,'/art_regression_outliers_and_movement_',name1,'.mat'], ...
    [path1,'/art_out_mov_',name1,output_suffix,ridx,'.mat']);
  
  % copy figures
  if isfield(EXP,'dir_figure')
    %screen2png([path1,'/art_plot',output_suffix,ridx,'.png'],72);
    
    [~,~]=mkdir(EXP.dir_figure);
    copyfile([path1,'/art_plot',output_suffix,ridx,'.png'], ...
      [EXP.dir_figure,'/art_plot',output_suffix,'_',subjid,'_',ridx,'.png']);
  end
  close(gcf);
...
% count outliers
load([path1,'/art_out_',name1,output_suffix,ridx,'.mat']);
numOut=size(R,2);
disp([subjid,': ',num2str(numOut)]);
